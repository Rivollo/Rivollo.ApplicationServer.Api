...
class ProductCreateWithImageUrls(BaseModel):
    ...
    The 3D model is generated by fal.ai SAM-3 (``fal-ai/sam-3/3d-objects``), whose
    input schema is just the image and the mask. The old self-hosted SAM tuning
    params (``target_format``, ``quality``, ``with_mesh_postprocess``,
    ``with_texture_baking``, ``use_vertex_color``, ``simplify``, ``fill_holes``,
    ``texture_size``) have no counterpart there and were removed rather than
